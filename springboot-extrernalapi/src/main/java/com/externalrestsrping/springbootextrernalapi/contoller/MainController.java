...
public class MainController {
	...
	public ModelAndView home15(@RequestParam(defaultValue = "") String name) {
		...
		try {
			List<City> cities = mapper.readValue(inputStream, typeReference).subList(0, 21);
	...
	public ModelAndView addCitie(@PathVariable(name = "name") String name, @Valid City ciity, ModelMap model) {
		ModelAndView modelAndView = new ModelAndView();
		...
		try {
			...
			City cityy = cities.stream().filter(city -> city.getName().equals(name)).findAny().orElse(null);
	...
	@PostMapping(value = "/addcities/{name}")
	...
	// save after edit
	@PutMapping(value = "/addcity/{id}/{name}")
	public ModelAndView addBeerEdit(@PathVariable(name = "id") Integer id, @PathVariable(name = "name") String name,
			@Valid City city, ModelMap model) {
		ModelAndView modelAndView = new ModelAndView();
	...
	public ModelAndView addBeerEdi(@PathVariable(name = "id") Integer id, @PathVariable(name = "name") String name,
			@Valid City city, ModelMap model) {
		ModelAndView modelAndView = new ModelAndView();
		System.out.println("Imeee" + name);
		City cityy = cityService.get(id);
	...
	public ModelAndView deleteCit(@PathVariable(name = "id") Integer id, ModelMap model) {
		cityService.delete(id);
		return new ModelAndView("redirect:/edit", model);
	}
